...
actions() ->
  [
    {"actions", {{name}}_cli_handler_actions, "List available actions"},
    {"help", {{name}}_cli_handler_help, "Print help"},
    {"proxy", {{name}}_cli_handler_proxy, "Proxy management"}
  ].
...
dispatch_action_parse(Module, Args) ->
  ...
  case getopt:parse(extract_getopt_config(MergedConfig), Args) of
    ...
    {error, {Reason, _Data}} ->
      {{name}}_cli:exit(?EXIT_CODE_GETOPT_FAILED, "Failed to parse command: ~p", [Reason])
  end.
